...
void gemm_fp16_norelu(torch::Tensor Ga, torch::Tensor Gb0,  torch::Tensor Gd0){
  cutlass::gemm::GemmCoord problem_size_0(Ga.sizes()[0], Gb0.sizes()[1], Ga.sizes()[1]);
...
void gemm_fp16(torch::Tensor Ga, torch::Tensor Gb0,  torch::Tensor Gd0, int activate) {
    if (activate == 0){
