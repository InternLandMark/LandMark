...
/*
Access both input and output in vectorized half2
*/
__global__ void encoding_pipelined_column_major_movedata_freq2_2x2x(
        const uint64_t num_elements, //所有执行的 threads的个数 行*列
        const uint64_t n_frequencies,
        const uint64_t col_encoding_offset,
        const uint64_t col_data_offset,
        ...
        half2* data_out)
{
  ...
  half2 inputs_freq0 = (data_in + j*data_height/2 + i_out)[0];


    /*
    move original data to output
    */
  (data_out + (col_data_offset + j)*data_height/2 + i_out)[0] = inputs_freq0;
  /*
    sin                          cos
    freq0 freq1 ...........      freq0 freq1 ...........
    =======================      =======================
  ...
  // sin freq0
  (data_out +  (col_encoding_offset+2*j)*data_height/2 + i_out)[0] = h2sin(inputs_freq0);
  // sin freq1
  (data_out + (col_encoding_offset+j*2+1)*data_height/2 + i_out)[0] = h2sin(inputs_freq1);

  // cos freq0
  (data_out + (col_encoding_offset+j*2 + indata_width*n_frequencies)*data_height/2 + i_out)[0] = h2cos(inputs_freq0);
  // cos freq0
  (data_out + (col_encoding_offset+j*2+1+ indata_width*n_frequencies)*data_height/2 + i_out)[0] = h2cos(inputs_freq1);
}


/*
Access both output in vectorized half2
*/
