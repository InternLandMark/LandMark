...
void launch_grid_sample_3d_float(float *input, float *sample_grid, float *output, int n, int d,
                                 int h, int w, int c, int batch_size, bool output_ncdhw, bool isOptExp, bool isBoundJudge,
                                 cudaStream_t stream);


void grid_sample(torch::Tensor input, torch::Tensor grid, torch::Tensor output, bool output_ncdhw, bool isOptExpr, bool isBoundJudge) {
  ...
  if (i_shape.size() == 5) {
    ...
    case c10::ScalarType::Float:
      launch_grid_sample_3d_float(input.data_ptr<float>(), grid.data_ptr<float>(),
                                  output.data_ptr<float>(), n, d, h, w, c, batch_size, output_ncdhw, isOptExpr, isBoundJudge,
                                  stream);
      break;
    default:
      throw;
