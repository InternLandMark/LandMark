...
__device__ float GridSample3D_2d_bool_dev(float* INPUT, unsigned long W_in, unsigned long H_in, unsigned long D_in, float ix, float iy, float iz, unsigned long channel_offset)
{
...
__global__ void func_GridSample3D_2d_bool_dev_kernel0(bool* __restrict__ OUTPUT, float* __restrict__ INPUT, float* __restrict__ GRID,
float* __restrict__ aabb, float* __restrict__ grid_aabb,
int D_out, int Batchsize, int Channel, int H_out, int W_out, int D_in, int H_in, int W_in, float alpha_thre) {
    if (((((int)blockIdx.x) * blockDim.x) + ((int)threadIdx.x)) < D_out && ((int)threadIdx.y) < H_out) {
        ...
        // aabb [[-19.2500,  -8.5000,  -1.0000], [ -5.5000,   8.5000,   2.0000]]
        // float aabb[2] = {-19.2500,  -8.5000};

        // alphaMask: grid_aabb [[-22., -17.,  -1.], [ 22.,  17.,   2.]]
        // int grid_aabb[2] = {-22, -17};
        // printf("%lld\t",grid_aabb[0]);



        float x = GRID[grid_offset];

        float aabbSize = (aabb[0+3] - aabb[0]) / 2;
        float gridSize = (grid_aabb[0+3] - grid_aabb[0]) / 2; // Note:(x-grid[0])/gridsize - 1 = x /gridsize if grid[0] = -gridsize
        // x = ((x * 6.875) - 19.250 + 6.875) / 22;
        x = ((x * aabbSize) + aabb[0] + aabbSize) / gridSize;
        float ix = grid_sampler_compute_source_index(x, W_in);

        float y = GRID[grid_offset + 1];
        aabbSize = (aabb[1+3] - aabb[1]) / 2;
        gridSize = (grid_aabb[1+3] - grid_aabb[1]) / 2; // Note:(x-grid[0])/gridsize - 1 = x /gridsize if grid[0] = -gridsize
        // y = ((y * 8.5)) / 17;
        y = ((y * aabbSize)) / gridSize; //if aabb[0] = -aabbSize & grid[0] = -gridsize
        ...
        OUTPUT[(((((((((int)blockIdx.x) * blockDim.x) + (channel_offset * D_out)) + ((int)threadIdx.x)) * H_out) + ((int)threadIdx.y)) * W_out) + ((int)threadIdx.z))] = GridSample3D_2d_bool_dev(INPUT, W_in, H_in, D_in, ix, iy, iz, channel_offset) > alpha_thre;
    }
}
...
int valid_samples_num, float alpha_thre, int blockdim)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(input.type(), "GridSample3D_2d_bool_dev", ([&] {
        func_GridSample3D_2d_bool_dev_kernel0<<<grid_shape, block_shape>>>(
            output.data<bool>(),
            input.data<float>(),
            grid.data<float>(),
            aabb.data<float>(),
            alphaMask_aabb.data<float>(),
            D_out,
            Batchsize,
            Channel,
            H_out,
