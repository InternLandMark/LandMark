        ...
        float* data_out)
{
        ...
        if (j<input1_width){ // fill feature data
            data_out[j+ i*output_width] = data_in1[j + i*input1_width];
        }
        else if (j<input1_width + input2_width){ // fill viewdir data
            input_j = j-input1_width;
            data_out[j+ i*output_width] = data_in2[input_j + i*input2_width];
        }
            ...
            data_out[j+ i*output_width] = (float)__sinf(input);
        }
...
torch::Tensor pe_concate(torch::Tensor data_in1,  int n_freq1, torch::Tensor data_in2, int n_freq2){

    const int indata_height = data_in1.size(0);
    const int indata_width1 = data_in1.size(1);
    const int indata_width2 = data_in2.size(1);
    const int output_width = indata_width1 + indata_width2 + indata_width1*n_freq1*2 + indata_width2*n_freq2*2;
    const uint64_t num_elements = indata_height * output_width;

    torch::Tensor data_out = torch::zeros({indata_height, output_width}, data_in1.options());
    const uint32_t gridDIM = (num_elements + blockDIM-1)/blockDIM;

    AT_DISPATCH_FLOATING_TYPES(
         data_in1.type(), "pe_concate_kernel", ([&] {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  py::arg("data_in1"), py::arg("n_freq1"), py::arg("data_in2"), py::arg("n_freq2"));
}
